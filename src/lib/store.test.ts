...
describe("resolveEnvRef", () => {
  ...
  it("should return value as-is if not an env reference", () => {
    ...
    expect(resolveEnvRef("https://api.anthropic.com")).toBe("https://api.anthropic.com");
...
describe("modelValueSchema", () => {
  ...
  it("should accept optional API_TIMEOUT_MS", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.API_TIMEOUT_MS).toBe(30000);
    }
  });
...
describe("Store operations", () => {
  ...
  describe("getDefaultModel", () => {
    it("should return model marked as default", () => {
      const models: ModelsJson = {
        "model-1": createTestModel({ name: "model-1", isDefault: false, order: 1 }),
        "model-2": createTestModel({ name: "model-2", isDefault: true, order: 2 }),
      };
      writeModels(models);
  ...
  describe("migrateModels", () => {
    ...
    it("should skip existing models", () => {
      ...
      const sourceModels: ModelsJson = {
        existing: createTestModel({ name: "existing", description: "different" }),
      };
