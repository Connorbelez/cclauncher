...
import { parseArgs, type CliCommand } from "./cli";
  ...
  describe("Add command", () => {
    it("should return add type with model data", () => {
      ...
      if (result.type === "add") {
        ...
        expect(result.model.value?.ANTHROPIC_BASE_URL).toBe("https://api.test.com");
    ...
    it("should set fast-model to model-id if not specified", () => {
      ...
      expect(result.type).toBe("add");
      if (result.type === "add") {
        expect(result.model.value?.ANTHROPIC_SMALL_FAST_MODEL).toBe("claude-3-opus");
      }
    });
    ...
    it("should use explicit fast-model if provided", () => {
      ...
      expect(result.type).toBe("add");
      if (result.type === "add") {
        expect(result.model.value?.ANTHROPIC_SMALL_FAST_MODEL).toBe("claude-3-haiku");
      }
    });
  ...
  describe("Complex scenarios", () => {
    it("should handle real-world add command", () => {
      ...
      if (result.type === "add") {
        ...
        expect(result.model.value?.ANTHROPIC_BASE_URL).toBe("https://api.anthropic.com/v1");
        expect(result.model.value?.ANTHROPIC_AUTH_TOKEN).toBe("env:ANTHROPIC_API_KEY");
        expect(result.model.value?.ANTHROPIC_MODEL).toBe("claude-3-opus-20240229");
        expect(result.model.value?.ANTHROPIC_SMALL_FAST_MODEL).toBe("claude-3-haiku-20240307");
